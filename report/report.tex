...
\resultnum{onsetmodelsestimatedk.targetaccept} in Analysis~4. Each fit jointly estimates $\Rpre$,
$\Rpost$, any latent states, and --- in Analyses~2 and~4 --- $k$. Because the estimand conditions
on the record through the day in question (Section~\ref{sec:smoothing}), each model is fitted once
per conditioning day, and the diagnostics below cover all of those fits. The evidence and
dispersion comparisons are properties of a model given the whole record, and use the fit to the
complete series, which also serves as the final conditioning day.
...
\resultnum{overall.diagnostics.minbulkess}, and the total number of divergent transitions is
