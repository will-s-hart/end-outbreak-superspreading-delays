...
def test_analysis_window_is_111_days_totalling_54_cases(data):
    assert data.n_days == 111
    assert data.total_cases == outbreak_data.TOTAL_CASES == 54


def test_window_runs_from_first_onset_to_ert_withdrawal(data):
    assert data.date_of(0) == outbreak_data.FIRST_ONSET_DATE == datetime.date(2018, 4, 5)
    assert data.date_of(-1) == outbreak_data.ERT_WITHDRAWAL_DATE == datetime.date(2018, 7, 24)
...
def test_pre_and_post_ert_case_totals(data):
    ...
    assert data.post_ert_mask.sum() == 78
...
def test_last_observed_onset_is_followed_by_52_zero_days(data):
...
def test_withdrawal_day_must_index_the_final_row():
