...
\subsection{Retrospective conditioning of latent states}
\label{sec:smoothing}

The estimand is deliberately retrospective. For \ssi{}, \sseso{} and \ssiso{}, the state
retained at day $t$ is therefore drawn from its \emph{smoothed} posterior conditional on the
complete record, not from the filtering distribution conditional only on observations through
$t$. Consequently, $\RAC(t)$ should not be interpreted as a real-time probability for these
models. Future observations can move the retained state in either direction: later cases imply
greater infectivity in earlier cohorts, whereas later zeros imply less. We compare smoothed and
filtering risks with particle methods as a sensitivity analysis. \dlo{} and \sse{} have no
latent state and are unaffected by this distinction.

\subsection{Recovering integrated-out latent states}
\label{sec:reconstruction}

Some latent states can be integrated out of the likelihood analytically (Section~\ref{sec:latent-block})
but are still needed to define the reset state. Before calculating either risk, we recover these
states from their exact conditional distribution,
$\Gam(k\,\mathrm{scale}_u,\ k+c_u)$, conditional on each posterior draw. This restores the complete
latent trajectory, including infections after the final observed onset that may remain in the
incubation pipeline.
...
\resultnum{onsetmodelsestimatedk.targetaccept} in Analysis~4. One fit per model and analysis
jointly estimates $\Rpre$, $\Rpost$, any latent states, and --- in Analyses~2 and~4 --- $k$.
...
\begin{itemize}
  \item \textbf{Exact marginalisation.} A latent that reaches no observation day carrying a case
    ...
    \resultnum{onsetmodelsfixedk.ssi.latents.total}. Section~\ref{sec:reconstruction} describes
    their exact recovery for the risk calculation.
...
marginal Metropolis--Hastings reproduces the parameter posteriors on synthetic data. Filtering
and smoothed particle states are kept distinct because they correspond to different estimands
(Section~\ref{sec:smoothing}). Bridge-sampling estimates agree with deterministic quadrature for
...
\begin{enumerate}
  ...
  \item \textbf{Filtering rather than smoothed latent states.} Integrate the filtering comparison
    of Section~\ref{sec:smoothing} over parameter uncertainty for all three latent-variable
    models.
